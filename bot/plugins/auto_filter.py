...
async def auto_filter(bot, update):
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("ꁅꂦ ꓄ꂦ ꈤꍟꊼ꓄ ᖘꍏꁅꍟ -->>", callback_data=f"navigate(0|next|{query})")
                ]
            )
        
        # Just A Decaration
        result[0].append([
            InlineKeyboardButton(f" ⭕️ꎭꍏꊼꀤꎭꀎꎭ ᖘꍏꁅꍟꌗ 1/{len_result if len_result < max_pages else max_pages} ⭕️", callback_data="ignore")
        ])
